...
let _ =
    ...
    let pid = Unix.create_process "/usr/bin/python" [|"python"; "symbols.py"|] pr1 pw2 Unix.stderr in
    ...
    let add_to_map token = 
        let ni = Str.split (Str.regexp ">") token in
        let name = List.nth ni 0 in 
        let add = List.nth ni 1 in
        let address = int_of_string add in
        let _ = Hashtbl.add fUNC_MAP name address in
        () 
    in
    let a = List.map add_to_map tokens in
    ()
    ...
    let rewrite stmt = 
        ...
        | ExtCall(id, ExpArgs(exp_args)) -> 
            ...
            let jmp_over_locals locals_filler = 
                ...
                [mov;ops]
            in
            let imp_addr = Hashtbl.find fUNC_MAP id in
...
let dump_pairs pairs = 
    ...
    let pr acc (instr,gmeta) = 
        let GMeta(_,fm,_,stack_fix) = gmeta in
        let (off, sep, off_s) = 
            if is_lbl_or_comment instr then
                acc, " ", 0
            else
                ...
                acc+stack_fix, "\t", off_s
        in
        let _ = printf "0x%04x%s%s\t0x%08x\n" acc sep (dump_instr instr) off_s in
        off
    in
