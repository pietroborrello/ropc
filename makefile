...
PACKS = bigarray,str,ocamlgraph,unix,camomile,threads,str
...
ropc: common.cmo lexer.cmo cdefs.cmo analysis.cmo main.cmo 
	ocamlfind ocamlc -package extlib,unix,str -linkpkg -o ropc common.cmo ast.cmo lexer.cmo parser.cmo cdefs.cmo analysis.cmo main.cmo
...
main.cmo: lexer.cmo parser.cmo analysis.cmo main.ml
	ocamlfind ocamlc -package extlib,str  -c $(OPTS) main.ml
...
asm: test.asm test.c
	...
	gcc -m32 asm.o test.c 

vg: asm 
	./gadget a.out candidates.bin
	./verify candidates.bin vg.bin
	./ropc examples-ropl/print.ropl vg.bin
